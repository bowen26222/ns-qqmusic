...
// ============================================================================
// 崩溃/流程日志器（补丁）：sdmc:/qqmusic-crash.log
//
// ovlloader 进程跨多次 ovl 会话存活，且自身堆只有 32KB（__libnx_initheap
// 的 g_innerheap）——日志路径全程 raw-fs + 栈缓冲，不用 stdio/malloc。
// 覆盖 ovl 侧日志器够不到的窗口：g_nroAddr==0（loadNro unmap/map 之间）、
// ovlmenu 阶段、ovlloader 自身代码。loadNro 各阶段标记 + 异常入口第一现场
// 合起来可精确定位蓝屏发生在哪个阶段、哪个模块。
// ============================================================================

// 内核异常帧 = ThreadExceptionFrameA64（ovl/sysmodule 侧同一布局）。
typedef struct {
    u64 gprs[9];
    u64 lr;
    u64 sp;
...
static volatile u32 s_ovll_log_in = 0;

static void ovllFsAppend(const char *data, u32 len) {
    svcOutputDebugString(data, len);
    ...
    rc = fsFsOpenFile(&g_sdmc, "/qqmusic-crash.log", mode, &fd);
    if (R_FAILED(rc)) {
        rc = fsFsCreateFile(&g_sdmc, "/qqmusic-crash.log", 0, 0);
        if (R_SUCCEEDED(rc))
            rc = fsFsOpenFile(&g_sdmc, "/qqmusic-crash.log", mode, &fd);
        if (R_FAILED(rc))
            goto failed;
    ...
    s64 size;
    rc = fsFileGetSize(&fd, &size);
    if (R_SUCCEEDED(rc))
        rc = fsFileWrite(&fd, size, data, len, FsWriteOption_Flush);
    fsFileClose(&fd);
failed:
    if (R_FAILED(rc)) {
        char error[80];
        snprintf(error, sizeof(error), "OVLL log write failed rc=0x%08x\n", (unsigned)rc);
        svcOutputDebugString(error, strlen(error));
    }
}

void ovllLog(const char *fmt, ...) {
    char buf[256];
    va_list ap;
    va_start(ap, fmt);
    ...
    if (n <= 0)
        return;
    ovllFsAppend(buf, (u32)((size_t)n < sizeof(buf) ? (size_t)n : sizeof(buf) - 1));
}

// 由 trampoline.s 异常入口调用：x0 = 帧指针，x1 = g_nroAddr（可能为 0）。
// ovlloader 进程跨会话存活 → guard 只防重入，写完即清（后续崩溃继续可记）。
void ovllCrashLog(u64 frame, u64 base) {
    if (__atomic_exchange_n(&s_ovll_log_in, 1, __ATOMIC_SEQ_CST))
        return;
    const OvllFrame *fr = (const OvllFrame *)frame;
    ovllLog("OVLL X pc=0x%llx lr=0x%llx sp=0x%llx far=0x%llx esr=0x%x pstate=0x%x base=0x%llx\n",
            (unsigned long long)fr->elr,
            (unsigned long long)fr->lr,
            (unsigned long long)fr->sp,
            (unsigned long long)fr->far,
            (unsigned int)fr->esr,
            (unsigned int)fr->pstate,
            (unsigned long long)base);
    __atomic_store_n(&s_ovll_log_in, 0, __ATOMIC_SEQ_CST);
}

// 供 main.c 之外的 .s 引用声明（避免依赖头文件）。
...
void __appInit(void) {
    ...
    ovllLog("OVLL diag=2 boot heap=0x%llx hos=0x%llx\n",
            (unsigned long long)g_appletHeapSize, (unsigned long long)s_hosVersion);
    smExit();
}

void __wrap_exit(void) {
    // Open/init SD card if needed
    ovllLog("OVLL wrap_exit\n");
    if (!g_sdmc_initialized) {
...
void loadNro(void) {
    ...
    ovllLog("LD start\n");
    if (g_nroSize > 0) {
        ovllLog("LD unmap base=0x%llx size=0x%llx\n",
                (unsigned long long)g_nroAddr, (unsigned long long)g_nroSize);
        // Unmap previous NRO
        header = &g_nroHeader;
        ...
        if (R_FAILED(rc)) {
            ovllLog("LD fatal unmap0 rc=0x%x\n", rc);
        ...
        if (R_FAILED(rc)) {
            ovllLog("LD fatal unmap1 rc=0x%x\n", rc);
        ...
        if (R_FAILED(rc)) {
            ovllLog("LD fatal unmap2 rc=0x%x\n", rc);
    ...
    ovllLog("LD read nro=0x%llx bss=0x%llx\n",
            (unsigned long long)header->size, (unsigned long long)header->bss_size);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (offset >= header->size) {
            ovllLog("LD fatal segv off=0x%x\n", offset);
        ...
        if (size > header->size - offset) {
            ovllLog("LD fatal segv size=0x%x\n", size);
            fatalThrow(MAKERESULT(Module_HomebrewLoader, 6));
        }
    }
    ...
    if (R_FAILED(rc)) {
        ovllLog("LD fatal perm1 rc=0x%x\n", rc);
    ...
    if (R_FAILED(rc))
    if (R_FAILED(rc)) {
        ovllLog("LD fatal perm2 rc=0x%x\n", rc);
    ...
    ovllLog("LD ready base=0x%llx size=0x%llx\n",
            (unsigned long long)g_nroAddr, (unsigned long long)g_nroSize);
