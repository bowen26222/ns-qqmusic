...
namespace qqmusic::impl {
    ...
    namespace {
        ...
        Result PlayTrack(const char* path) {
            ...
            R_UNLESS(source != nullptr, qqmusic::FileOpenFailure);
            R_UNLESS(source->IsOpen(), qqmusic::FileOpenFailure);
    ...
    Result GetCurrentQueueItem(CurrentStats *out, char *buffer, size_t buffer_size) {
        ...
        R_UNLESS(g_source != nullptr, qqmusic::NotPlaying);
        R_UNLESS(g_source->IsOpen(), qqmusic::NotPlaying);

        const auto path = g_playlist.GetPath(g_current);
        R_UNLESS(path, qqmusic::NotPlaying);
        std::snprintf(buffer, buffer_size, "%s", path);
